...
@property(nonatomic,retain)ItemPageView2* itemPageView;
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        itemPageView = [[ItemPageView2 alloc] init];
        self.itemPageView.delegate = self;
        self.itemPageView.countPerPage = 8;
        self.itemPageView.sizePerItem = CGSizeMake(114, 70);
        self.itemPageView.sameSapcer = NO;
        self.itemPageView.hasPageControll = YES;
        self.itemPageView.paddingSize = CGSizeMake(30, 15);
...
//            NSLog(@"%@--%@",self.dataList ,self.lastSuggestArray);
...
//                NSArray *a = self.lastSuggestArray;
//                if ([self.suggestFieldText isEqualToString:[a objectAtIndex:[a count]-1]]) {
////                    [dataList reloadShowedDataWithIDList:self.selectID];
////                    [dataList addCommnetContents:a IDList:self.selectID];
//                }
...
//            NSMutableArray *tem_arr = [NSMutableArray arrayWithArray:self.suggestArray];
//            [tem_arr addObject:self.suggestFieldText];
//            
//            self.lastSuggestArray = tem_arr;
...
//    nameLabel.text = [object valueForKey:StockFunc_RemindStockInfo_name];
...
{
    PageItemCell2* cell = [[[PageItemCell2 alloc] init] autorelease];
    NewsObject* oneObject = [self.configDataList oneObjectWithIndex:index+1 IDList:nil];
...
//    [self searchBarSearchButtonClicked:curSearchBar];
    ...
    NSString* symbol = str;//[Util get_total_symbol_string:str];
...
//                NSString *curMode=[NSString stringWithFormat:@"%@",[[UIScreen mainScreen] preferredMode]];
...
//                            possibleKeyboard = [[possibleKeyboard subviews] objectAtIndex:1];
//                        }else{
//                            possibleKeyboard = [[possibleKeyboard subviews] objectAtIndex:0];
//                        }
//
