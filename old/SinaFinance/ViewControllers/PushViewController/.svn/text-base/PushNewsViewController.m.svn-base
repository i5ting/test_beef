...
+ (PushNewsViewController *)sharedInstance {
    static PushNewsViewController *sharedAppDelegate = nil;
...
//        [addStockView addSubview:historyBtn];
    ...
    if (!self.dataTableView) {
        ...
        CommentDataList* tixingIDList = app.tixingIDList;
        dataView.dataList = app.tixingIDList;
...
-(UITableViewCell*)stockHistoryCellWithView:(DataListTableView*)view cellForIndexPath:(NSIndexPath*)path object:(NewsObject*)object
{
    ...
    rtval.hideRemove = YES;
     ...
     int rowNum = [path row];
    int Count = [dataList contentsCountWithIDList:nil];
    NSArray* is_read_data_array = (NSArray*)[SaveData getDataForKey:[Util get_tixing_has_readed_file_name] ];
    
    if (is_read_data_array && ([is_read_data_array count] >= Count) ) {
        if ([is_read_data_array count]>rowNum) {
...
-(void)dataListView:(DataListTableView*)view didSelectedAtIndexPath:(NSIndexPath*)path object:(NewsObject*)object
{
    ...
    if (view==self.newsTableView)
    {
        ...
        int rowNum = [path row];
        [self update_tixing_table_with_index:rowNum news_object:object];
        NSString* remindKey = [self.selectID objectAtIndex:1];
        if ([remindKey isEqualToString:StockRemindSettingKey]) {
...
-(void)update_tixing_table_with_index:(int)index new_value:(NSString *)newValue{
    ...
    //如果没有颜色配置文件的话，默认为NO
    [SaveData saveDataForKey:[Util get_tixing_has_readed_file_name] data:newArray];
    [newArray release];
...
-(void)update_tixing_table_with_index:(int)index news_object:(NewsObject *)oneObject{
    ...
    if (!is_read_data_array) {
        ...
        NSLog(@"INFO:0  tixingArray= %d:%@",[newArray count],newArray);
        ...
        NSLog(@"INFO:1  tixingArray= %d:%@",[newArray count],newArray);
