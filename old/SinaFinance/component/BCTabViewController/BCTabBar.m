...
-(void)setSelectedTabNoAnimate:(BCTab *)aTab;
@end
    ...
    if (tabs != array) {
        ...
        if ([array count]>0) {
            ...
            if (self.preData) {
                ...
                [self layOutScrollView];
                [self resetScrollPosWithPos:newContentPos selectedId:newSelectedIndex];
            }
            else
            {
                ...
                [self setSelectedTabNoAnimate:[self.tabs objectAtIndex:column]];
...
- (void)setSelectedTab:(BCTab *)aTab {
    BOOL bSelectedSameTab = self.selectedTab==aTab?YES:NO;
	[self setSelectedTab:aTab animated:YES];
    if(!bSelectedSameTab&&[self.delegate respondsToSelector:@selector(tabBar:didSelectTabAtIndex:byBtn:)])
    {
        [self.delegate tabBar:self didSelectTabAtIndex:[self.tabs indexOfObject:aTab]byBtn:NO];
    }
}
...
- (void)tabSelected:(BCTab *)sender {
    if (self.userInteractionEnabled&&!self.hidden) {
        [self tabSelected:sender byBtn:YES];
    }
}
   
- (void)tabSelected:(BCTab *)sender byBtn:(BOOL)byBtn
{
    if ([self.tabs count]>0) {
        BOOL bSelectedSameTab = self.selectedTab==sender?YES:NO;
        [self setSelectedTab:sender animated:YES];
        
        if (byBtn&&[self.delegate respondsToSelector:@selector(tabBar:didSelectTabAtIndex:byBtn:)]) {
            [self.delegate tabBar:self didSelectTabAtIndex:[self.tabs indexOfObject:sender]byBtn:byBtn];
        }
        else if(!byBtn&&[self.delegate respondsToSelector:@selector(tabBar:didSelectTabAtIndex:byBtn:)]&&!bSelectedSameTab)
        {
            [self.delegate tabBar:self didSelectTabAtIndex:[self.tabs indexOfObject:sender]byBtn:byBtn];
        }
    }
...
-(void)resetScrollPosWithPos:(CGPoint)newPos selectedId:(NSInteger)index
{
    if (!CGPointEqualToPoint(newPos, CGPointZero)||index==0) {
        self.scrollPos = newPos;
    }
    else
    {
        BCTab* oneTab = [tabs objectAtIndex:index];
        CGRect tabFrame = oneTab.frame;
        CGPoint retPos = CGPointMake(tabFrame.origin.x, tabFrame.origin.y);
        if (selectedMovement==MovementStyle_Free) {
            CGSize contentSize = self.scrollView.contentSize;
            CGRect contentRect = self.scrollView.bounds;
            if (retPos.x>contentSize.width-contentRect.size.width) {
                retPos.x = contentSize.width-contentRect.size.width;
            }
            if (retPos.y>contentSize.height-contentRect.size.height) {
                retPos.y = contentSize.height-contentRect.size.height;
            }
        }
        self.scrollPos = retPos;
    }
}
...
-(void)setSelectedColumnWithReload:(NSInteger)index
{
    ...
    if ([self.tabs count]>index) {
        ...
        if(!bSelectedSameTab&&[self.delegate respondsToSelector:@selector(tabBar:didSelectTabAtIndex:byBtn:)])
        {
            [self.delegate tabBar:self didSelectTabAtIndex:[self.tabs indexOfObject:tab]byBtn:NO];
        }
    }
}
...
-(BOOL)adjustScrollViewContentOffset
{
    ...
    if (self.tabs) {
        if (selectedMovement!=MovementStyle_Free) {
            ...
            if(self.selectedTab!=findTab)
            {
                [self tabSelected:findTab byBtn:NO];
            }
        }
    }
    return rtval;
...
-(BCTab*)findTabWithPos:(CGPoint)offsetPos
{
    ...
    for (int i=0;i<=tabCount;i++) {
        ...
        {
            secondTab = [self.tabs objectAtIndex:i];
        }
...
-(void)updateCoverArrowImage
{
        ...
        if (YES) {
            ...
            newRect.size = newSize;
             ...
             newRect.origin.x = oldRect.origin.x - (newSize.width - oldSize.width)/2;
                 ...
                 CGRect selectedFrame = self.selectedTab.frame;
                 newRect.origin.x = selectedFrame.size.width/2 - newRect.size.width/2 + selectedFrame.origin.x;
             }
    ...
    if (coverArrowUseText) {
        ...
        if (self.selectedTab)
        {
            ...
            coverArrowLabel.textColor = [UIColor whiteColor];
