...
-(void)reloadNilDataForSingleRow:(NSNumber*)rowNumber
{
    ...
    {
        if (_multiCount>0) {
            if([self.delegate respondsToSelector:@selector(multiTabBar:didSelectTabAtIndex:byBtn:)])
            {
                NSIndexPath* index = [NSIndexPath indexPathForRow:row-1 inSection:-1];
                [self.delegate multiTabBar:self didSelectTabAtIndex:index byBtn:byBtning];
            }
        }
    }
}
...
-(void)tabBar:(BCTabBar *)aTabBar didCoverArrowHideChanged:(BOOL)bHided
{
    NSInteger row = [self curRowForTabBar:aTabBar];
    if (!bHided) {
        BCTab* oldBar = aTabBar.oldSelectedTab;
        BCTab* selectedBar = aTabBar.selectedTab;
        if ((oldBar!=selectedBar||oldBar==nil)) {
