...
@implementation NewsListFuncPuller
@synthesize downloadQueue=mDownloadQueue;
@synthesize requester;

+ (id)getInstance
{
    static NewsListFuncPuller* s_messageManager = nil;
	if (s_messageManager == nil)
	{
		//没有创建则创建
		s_messageManager = [[NewsListFuncPuller alloc] init];
...
#ifdef DEBUG
    NSLog(@"urlStr newslist funcpuller = %@",urlStr);
#endif
...
- (void)requestFinished:(ASIHTTPRequest *)request
{
    ...
    if (jsonDict&&[jsonDict isKindOfClass:[NSDictionary class]]) {
        ...
        if ([codeValue intValue]==0) {
            NSMutableDictionary* dataDict = [resultDict valueForKey:@"data"];
            ...
            [tempArray addObjectsFromArray:scrollArray];
            ...
            if ([countNumber intValue]==[scrollArray count]) {
