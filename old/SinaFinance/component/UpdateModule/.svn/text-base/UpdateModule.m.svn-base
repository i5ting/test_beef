...
void UncaughtExceptionHandler(NSException *exception) {
    ...
    NSString *urlStr = [NSString stringWithFormat:@"mailto://fallbort@sina.cn?subject=bug报告&body=感谢您的配合!<br><br><br>"
                        "错误详情:<br>%@<br>--------------------------<br>%@<br>---------------------<br>%@", 
...
{
    NSTimer* timer;
    NSTimeInterval pastedTime;
    NSInteger timerPastedTime;
}
...
{
    if (!timer) {
        [self performSelector:@selector(firstStartChekUpdate) withObject:nil afterDelay:3.0];
        timer = [NSTimer scheduledTimerWithTimeInterval:1.0 target:self selector:@selector(CheckUpdateTimer:) userInfo:nil repeats:YES];
    }
}

-(void)CheckUpdateTimer:(NSTimer*)sender
{
    timerPastedTime += timer.timeInterval;
    if (timerPastedTime%10==0) {
        [self realCheckUpdateTimer];
    }
}

-(void)realCheckUpdateTimer
{
    NSMutableDictionary* oldVersion = [[RegValueSaver getInstance] readSystemInfoForKey:LastTipUpdateVersion];
    ...
    if (oldVersion&&runningVer) {
        ...
        NSInteger newPast = (curtime - [timeNumber doubleValue]);
        if (timerPastedTime%20==0&&(newPast<pastedTime||newPast<0)) {
            pastedTime = 0;
            ...
            if ((curtime - [timeNumber doubleValue])>12*60*60) {
                pastedTime = 0;
                [self startCheckUpdate:NO];
            }
        }
...
-(BOOL)hasNewVersion:(NSString*)version
{
    BOOL rtval = NO;
    NSString* bundleVersion = [[[NSBundle mainBundle] infoDictionary] objectForKey:@"CFBundleVersion"];
    NSString* appVersion = [[NSString alloc] initWithFormat:@"%@",bundleVersion];
