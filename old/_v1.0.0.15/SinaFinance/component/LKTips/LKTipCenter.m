...
-(void)realShowTopTipWithMessage
{
    if (!showingTopTip&&topMessageArray&&[topMessageArray count]>0) {
        ...
        if (!CGRectEqualToRect(statusFrame,CGRectZero)) {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:TopTipStartNotification object:nil];
...
-(void)startToptipHide
{
    ...
    else
    {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:TopTipEndNotification object:nil];
...
{
    [self postCenterTipWithMessage:message time:dTime ignoreAddition:ignore pView:view font:nil];
}

-(void)postSmallCenterTipWithMessage:(NSString*)message time:(NSTimeInterval)dTime ignoreAddition:(BOOL)ignore pView:(UIView*)view
{
    [self postCenterTipWithMessage:message time:dTime ignoreAddition:ignore pView:view font:[UIFont systemFontOfSize:16.0]];
}

-(void)postCenterTipWithMessage:(NSString*)message time:(NSTimeInterval)dTime ignoreAddition:(BOOL)ignore pView:(UIView*)view font:(UIFont*)font
{
    if (!centertipView) {
        ...
        [aTipView release];
        CGRect backRect = CGRectMake(0, 0, 100, 100);
        backRect.origin.x = bounds.size.width/2 - backRect.size.width/2;
        backRect.origin.y = bounds.size.height/2 - backRect.size.height/2;
