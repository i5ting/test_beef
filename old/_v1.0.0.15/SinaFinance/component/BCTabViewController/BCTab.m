...
@interface BCTab (){
    UIImageView *edageImageView;
}

@end

@implementation BCTab
@synthesize forceImageView,mainImageView,nameLabel,selected,backgroundImageView;
@synthesize hasbgEdageImageView;
...
-(UIImageView*)backgroundImageView
{
    if (!backgroundImageView) {
        ...
        if (hasbgEdageImageView) {
            edageImageView = [[UIImageView alloc] initWithFrame:CGRectZero];
            edageImageView.backgroundColor = [UIColor clearColor];
            [backgroundImageView addSubview:edageImageView];
        }
    }
...
- (void)layoutSubviews {
    ...
    if (backgroundImageView) {
        ...
        if (hasbgEdageImageView &&mSelected) {
            edageImageView.image= [UIImage imageNamed:@"bg_selected"];
            
//            bgView.frame = CGRectMake(2.5f, 4, 55, 23);
            
            //                [oneTab addSubview:bgView];
            
            
            CGRect f =backgroundImageView.frame;
            f.origin.y += 4;
            f.origin.x +=2.5f;
            f.size.width -= 5;
            f.size.height -= 8;
            edageImageView.frame = f;
            
            [self addSubview:edageImageView];
            [self bringSubviewToFront:nameLabel];
        }else{
            [edageImageView removeFromSuperview];
        }
    }
    if (nameLabel) {
