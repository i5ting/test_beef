...
@property(retain)NSArray* defaultIDList;
...
+(NSArray*)convertNumberToString:(NSArray*)array createNewArray:(BOOL)created
{
    NSMutableArray* rtval = nil;
    for (int i=0; i<[array count]; i++) {
        NSString* oneObject = [array objectAtIndex:i];
        if ([oneObject isKindOfClass:[NSNumber class]]) {
            oneObject = [(NSNumber*)oneObject stringValue];
        }
        else if(i==0)
        {
            break;
...
+(BOOL)checkNumberArrayEqualWithFirstArray:(NSArray*)first secondArray:(NSArray*)second
{
    ...
    {
        rtval = NO;
        if ((!first&&!second)||(!first&&(second&&[second count]==0))||(!second&&(first&&[first count]==0))) {
            rtval = YES;
        }
    }
...
-(id)init
{
    ...
    if (self) {
        ...
        countPerPage = 20;
        defaultIDList = [[NSArray alloc] initWithObjects:@"{idlist}", nil];
    }
    return self;
}
...
-(id)initWithJsonString:(NSString*)jsonString
{
    ...
    if (self) {
        ...
        //mNameLock = [[NSLock alloc] init];
        mOrderLock = [[NSLock alloc] init];
        NSArray* commentArray = [jsonString objectFromJSONString];
...
-(id)initWithJsonArray:(NSArray*)jsonArray
{
    ...
    if (self) {
        ...
        if ([jsonArray isKindOfClass:[NSDictionary class]]) {
            jsonArray = [NSMutableArray arrayWithObject:jsonArray];
        }
...
-(void)initNameList
{
    NSArray* commentArray = [[JSONConfigDownloader getInstance] jsonOjbectWithJSONConfigFile:self.fileName];
        ...
        for (int i=0;i<nameListCount;i++) {
            ...
            if (self.nameListType==NameListType_queue) {
                ...
                for (int j=0; j<[firstArray count]; j++) {
                    ...
                    NSNumber* showedNumber = [(NSDictionary*)[firstArray objectAtIndex:j] valueForKey:@"defaultshow"];
                    if (showedNumber&&[showedNumber intValue]==0) {
                        orderData.type = AdjustOrderType_disabled;
                    }
                    else {
                        orderData.type = AdjustOrderType_abled;
                    }
        ...
        for (int i=0; i<[self.nameList count]; i++) {
            ...
            if (orderArrayList) {
                ...
                for (int k=0; k<[oldTotalOrderArray count]; k++) {
                    AdjustOrderData* adjustData_Total = [oldTotalOrderArray objectAtIndex:k];
                    BOOL hasFound = NO;
                    ...
                    if (!hasFound) {
                        if (adjustData_Total.type==AdjustOrderType_abled) {
    ...
    if (columns) {
        for (int i=0;i<[columns count];i++) {
            ...
            if (destArray&&column<[destArray count]) {
                ...
                NSDictionary* paramDict = [oneValue objectForKey:@"otherparam"];
                otherParam = [self otherParamFromDict:paramDict sourceParam:otherParam];
                NSString* ortherApi = [oneValue objectForKey:@"api"];
                if (ortherApi) {
                    totalAPI = ortherApi;
                }
                NSString* dataKey = [oneValue objectForKey:@"datakey"];
...
{
    [self refreshCommnetContents:objects IDList:IDList dateUpdate:YES];
}
...
-(void)refreshDataBaseWithOneCommnetContent:(NewsObject*)oneObject dataIDList:(NSArray*)dataIDArray row:(NSInteger)row
{
    if (self.dataTableName) {
        ...
        if ([contentsArray count]>row) {
            NewsObject* oldObject = [contentsArray objectAtIndex:row];
    ...
    if (needReolad) 
    {
        ...
        if (savedContentsArray) {
            ...
            for (int i=0;i<[contentsArray count];i++) {
                NewsObject* oneObject = [contentsArray objectAtIndex:i];
...
{
    NSNumber* rtval = (NSNumber*)[self infoValueWithIDList:IDList ForKey:@"pageend"];
    if (rtval) {
        return [rtval boolValue];
    }
    else {
        return NO;
    }
...
{
    [self setInfoWithIDList:IDList value:[NSNumber numberWithBool:pageEnd] forKey:@"pageend"];
}

-(NSInteger)loadedStateInfoWithIDList:(NSArray*)IDList;
{
    NSNumber* rtval = (NSNumber*)[self infoValueWithIDList:IDList ForKey:@"loadedstate"];
    if (rtval) {
        return [rtval intValue];
    }
    else {
        return -1;
    }
...
{
    [self setInfoWithIDList:IDList value:[NSNumber numberWithInt:loadedState] forKey:@"loadedstate"];
}
